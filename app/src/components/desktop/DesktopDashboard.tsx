...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const {
    ...
    queryFn: () =>
      invoke<Array<{ id: number; name: string; size: number; icon_type: string; folder_id: number | null; created_at: string; mime_type?: string; file_ext?: string }>>("cmd_get_files", { folderId: activeFolderId }).then((res) =>
        res.map((f) => ({
          ...f,
  ...
  const displayedFiles = searchTerm.length > 2 ? searchResults : allFiles.filter((f: TelegramFile) => f.name.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const { downloadQueue, queueDownload, queueBulkDownload, clearFinished: clearDownloads, cancelAll: cancelDownloads, cancelItem: cancelDownloadItem, retryItem: retryDownloadItem, decryptRequest, submitPassphrase } = useFileDownload(store);
  const { handleDelete, handleBulkDelete, handleBulkDownload, handleBulkMove, handleDownloadFolder, handleGlobalSearch } = useFileOperations(activeFolderId, selectedIds, setSelectedIds, displayedFiles, queueBulkDownload);
  ...
  const handleDropOnFolder = async (e: React.DragEvent, targetFolderId: number | null) => {
    ...
    try {
      ...
      await Promise.all(
        idsToMove.flatMap((id) => [invoke("cmd_delete_image_thumbnail", { messageId: id, folderId: activeFolderId }).catch(() => {}), invoke("cmd_delete_preview_for_message", { messageId: id, folderId: activeFolderId }).catch(() => {})]),
      );
  ...
  const handleRootDragOver = (e: React.DragEvent) => {
    ...
    const isInternalDrag = internalDragRef.current !== null || e.dataTransfer.types.includes("application/x-telegram-file-id") || e.dataTransfer.types.includes("application/x-telegram-file-ids");
    if (isInternalDrag) {
      e.preventDefault();
      e.stopPropagation();
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-telegram-bg relative" onDragOver={handleRootDragOver} onDragEnter={handleRootDragEnter}>
      ...
      {renameFolder && <RenameFolderModal folderId={renameFolder.id} currentName={renameFolder.name} onRename={handleFolderRename} onClose={() => setRenameFolder(null)} />}

      {renameFileTarget && <RenameFileModal fileName={renameFileTarget.name} onRename={handleRenameSubmit} onClose={() => setRenameFileTarget(null)} />}

      {/* Bulk Share Results Modal */}
      {bulkShareLinks && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setBulkShareLinks(null)}>
          <div className="bg-telegram-surface border border-telegram-border rounded-xl w-[500px] max-h-[70vh] shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-150" onClick={(e) => e.stopPropagation()}>
            ...
            {bulkShareLoading ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-2 min-h-0">
                {bulkShareLinks.map(({ file, link }) => {
                  ...
                  return (
                    <div key={file.id} className="p-3 rounded-lg bg-telegram-hover/30 border border-telegram-border/30 space-y-2">
                      <p className="text-xs font-semibold text-telegram-text truncate">{file.name}</p>
                      <div className="flex gap-2">
                        <input type="text" readOnly value={link} className="flex-1 bg-telegram-bg border border-telegram-border rounded-lg px-2.5 py-1.5 text-xs text-telegram-text focus:outline-none select-all truncate" />
                        <button
                          ...
                          className={`px-2.5 py-1.5 rounded-lg border flex items-center justify-center transition-all flex-shrink-0 ${
                            isCopied ? "bg-emerald-500 border-emerald-500 text-white" : "bg-telegram-hover border-telegram-border text-telegram-text hover:bg-white/10"
                          }`}
                        >
            ...
            <button onClick={() => setBulkShareLinks(null)} className="w-full px-4 py-2.5 border-t border-telegram-border bg-telegram-hover/20 hover:bg-telegram-hover/40 text-telegram-text text-sm font-medium transition-colors">
              Done
            </button>
          </div>
        </div>

...
import { GpgSettings } from "./GpgSettings";
...
type SettingsTab = "general" | "proxy" | "vpn" | "sharing" | "about" | "gpg";
...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const handleGenerateKey = async () => {
    const ok = await confirm({
      ...
      message: apiSettings.key_set ? "This will revoke your current API key and generate a new one. Any existing integrations will stop working." : "Generate a new API key for authenticating REST API requests.",
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
          <motion.div
            layout
          ...
          >
            ...
            <div className="px-5 py-4 border-b border-telegram-border flex justify-between items-center">
              ...
              <button onClick={onClose} className="p-1.5 hover:bg-telegram-hover rounded-lg text-telegram-subtext hover:text-telegram-text transition">
                <X className="w-4 h-4" />
              </button>
            </div>
              ...
              {(
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-t-lg transition-colors ${
                    activeTab === key ? "text-telegram-primary border-b-2 border-telegram-primary bg-telegram-primary/5" : "text-telegram-subtext hover:text-telegram-text hover:bg-telegram-hover/50"
                  }`}
                >
            ...
            {/* Tab Bar - Diperbaiki agar bisa scroll */}
            <div className="px-5 pt-3 pb-0 border-b border-telegram-border overflow-x-auto scrollbar-hide">
                    ...
                    <section className="space-y-3">
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-telegram-text font-medium w-5 text-center">{settings.maxConcurrentUploads}</span>
                          <button
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-telegram-text font-medium w-5 text-center">{settings.maxConcurrentDownloads}</span>
                          <button
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-telegram-subtext">{apiSettings.running ? `Running on port ${apiSettings.port}` : "Localhost only (127.0.0.1)"}</p>
                          </div>
                        </div>
                        <button
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === "Enter") handlePortApply();
                            }}
                      ...
                      <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2.5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-telegram-subtext">{apiSettings.key_set ? "Key configured" : "No key set"}</p>
                            </div>
                          </div>
                          <button onClick={handleGenerateKey} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-telegram-primary/10 text-telegram-primary hover:bg-telegram-primary/20 transition">
                        ...
                        {generatedKey && (
                          <div className="mt-2 p-2.5 bg-telegram-bg rounded-lg border border-yellow-500/20">
                            <p className="text-[10px] text-yellow-400/80 uppercase tracking-wider font-semibold mb-1.5">Copy now — this key will not be shown again</p>
                            <div className="flex items-center gap-2">
                              <code className="flex-1 text-xs text-telegram-text font-mono bg-telegram-hover rounded px-2 py-1.5 overflow-x-auto select-all">{generatedKey}</code>
                              <button onClick={handleCopyKey} className="p-1.5 rounded-md hover:bg-telegram-hover text-telegram-subtext hover:text-telegram-text transition flex-shrink-0" title="Copy to clipboard">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-telegram-subtext">{transcodeCache ? `${(transcodeCache.total_bytes / 1048576).toFixed(1)} MB / ${(transcodeCache.max_bytes / 1073741824).toFixed(1)} GB` : "Loading..."}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <button onClick={fetchTranscodeCache} disabled={cacheLoading} className="p-1.5 rounded-md hover:bg-telegram-hover text-telegram-subtext hover:text-telegram-text transition" title="Refresh">
                                    ...
                                    {entries.map((e) => (
                                      <button
                                        ...
                                        className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-medium transition ${
                                          e.playlist_exists ? "bg-emerald-500/10 text-emerald-400 hover:bg-red-500/10 hover:text-red-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-400/60 border border-amber-500/20"
                                        } disabled:opacity-30`}
                      ...
                      <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-telegram-subtext">{updateVersion ? `v${updateVersion} available` : "Check if a newer version exists"}</p>
                            </div>
                          </div>
                          {updateAvailable && !updateDownloading ? (
                            <button onClick={handleInstallUpdate} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-telegram-primary text-white hover:bg-telegram-primary/90 transition">
                        ...
                        {updateDownloading && (
                          <div className="w-full h-1.5 bg-telegram-border rounded-full overflow-hidden">
                            <div className="h-full bg-telegram-primary rounded-full transition-all duration-300" style={{ width: `${updateProgress}%` }} />
                          </div>
                        )}
                      </div>
                    </section>
                ...
                {activeTab === "proxy" && (
                  ...
                  >
                    ...
                    <div className="p-3 rounded-lg bg-yellow-500/5 border border-yellow-500/10 space-y-2">
                      <p className="text-[11px] text-yellow-400/70 leading-relaxed">⚠️ Proxy changes require reconnecting to take effect.</p>
                      <button
                        onClick={async () => {
                ...
                {activeTab === "vpn" && (
                  ...
                  >
                      ...
                      {latencyMs !== null && (
                        <span
                          className={`ml-auto text-[10px] font-mono px-1.5 py-0.5 rounded-full ${
                            latencyMs < 0 ? "bg-red-500/10 text-red-400" : latencyMs < 100 ? "bg-green-500/10 text-green-400" : latencyMs < 300 ? "bg-yellow-500/10 text-yellow-400" : "bg-red-500/10 text-red-400"
                          }`}
                        >
                    ...
                    <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                      ...
                      <button onClick={() => updateSetting("vpnMode", !settings.vpnMode)} className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${settings.vpnMode ? "bg-emerald-500" : "bg-telegram-border"}`}>
                      ...
                      <>
                        ...
                        <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2">
                          ...
                          <input
                            ...
                            max="5"
                            step="0.5"
                            value={settings.retryBaseBackoffSec}
                          ...
                          <input
                            ...
                            max="60"
                            step="2"
                            value={settings.retryMaxBackoffSec}
                        ...
                        <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2">
                          ...
                          <input
                            ...
                            max="4"
                            step="1"
                            value={settings.dcFallbackAttempts}
                        ...
                        <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-telegram-primary font-mono">{settings.bandwidthLimitUpKBs === 0 ? "Unlimited" : `${settings.bandwidthLimitUpKBs} KB/s`}</span>
                          </div>
                          <input
                            type="range"
                            min="0"
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-xs text-telegram-primary font-mono">{settings.bandwidthLimitDownKBs === 0 ? "Unlimited" : `${settings.bandwidthLimitDownKBs} KB/s`}</span>
                          </div>
                          <input
                            type="range"
                            min="0"
                        ...
                        <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-telegram-primary font-mono font-medium">{settings.keepAliveIntervalSec === 0 ? "Off" : `${settings.keepAliveIntervalSec}s`}</span>
                          </div>
                          <input
                            type="range"
                            min="0"
                            max="120"
                            step="15"
                            value={settings.keepAliveIntervalSec}
                        ...
                        <div className="p-3 rounded-lg bg-telegram-hover/50 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-telegram-primary font-mono font-medium">{settings.archiveMaxBytes === 0 ? "Unlimited" : `${settings.archiveMaxBytes} MiB`}</span>
                          </div>
                          <input
                            type="range"
                            min="0"
                            max="2048"
                            step="64"
                            value={settings.archiveMaxBytes}
                        ...
                        <div className="flex items-center justify-between p-3 rounded-lg bg-telegram-hover/50">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-telegram-subtext">{vpnDetected === true ? "VPN interface detected" : vpnDetected === false ? "No VPN detected" : "Checking..."}</p>
                            </div>
                          </div>
                          <button
                ...
                {activeTab === "sharing" && (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={fetchShares} className="text-telegram-subtext hover:text-telegram-text p-1 rounded hover:bg-telegram-hover transition" title="Refresh links">
                    ...
                    <div className="bg-telegram-hover/30 border border-telegram-border/50 rounded-lg p-3 space-y-2">
                      ...
                      />
                      <p className="text-[10px] text-telegram-subtext">Automatically replaces '127.0.0.1:14201' with this IP/domain when copying.</p>
                    </div>
                              ...
                              <div className="flex justify-between items-start gap-4">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="flex gap-2 items-center mt-1 flex-wrap text-[10px]">
                                    <span className="text-telegram-subtext">{new Date(share.created_at * 1000).toLocaleDateString()}</span>
                                    ...
                                    {share.expires_at ? (
                                      isExpired ? (
                                        ...
                                        <span className="text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded font-medium">Expires: {new Date(share.expires_at * 1000).toLocaleDateString()}</span>
                                      )
                                    ) : (
                ...
                {activeTab === "about" && (
                  ...
                  >
                    <div className="flex flex-col items-center py-6 space-y-5">
                      {/* Logo */}
                      <img src="/logo.svg" className="w-16 h-16 drop-shadow-lg" alt="Cicem Drive Logo" />

                      {/* App Name & Version */}
                      <div className="text-center">
                        <h3 className="text-base font-bold text-telegram-text">Cicem Drive</h3>
                      ...
                      >
                        {diagLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Clipboard className="w-3 h-3" />}
                        Copy Diagnostics
                      </button>
                      ...
                      <div className="text-center space-y-3">
                        <div>
                          <p className="text-sm font-semibold text-telegram-text">Ibnu Batuthah</p>
                        </div>
                        ...
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            open("https://github.com/ibnuasyi/Telegram-Drive-GPG");
                          }}
                          className="flex items-center justify-center gap-1.5 text-xs text-telegram-primary hover:text-telegram-primary/80 transition-colors cursor-pointer"
                        >
                          ...
                          github.com/ibnuasyi/telegram-drive-pgp
                        </button>
                      </div>

                      {/* Tagline */}
                      <p className="text-[11px] text-telegram-subtext/60 leading-relaxed max-w-[280px] text-center">Turn your Telegram account into unlimited, secure cloud storage. Open-source and free forever.</p>
                    </div>
                  </motion.section>
                )}
                {activeTab === "gpg" && (
            ...
            <div className="px-5 py-3 border-t border-telegram-border flex items-center justify-between">
              <button onClick={resetSettings} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs text-telegram-subtext hover:text-red-400 hover:bg-red-500/10 transition font-medium">
                <RotateCcw className="w-3.5 h-3.5" />
                Reset to Defaults
              </button>
              <button onClick={onClose} className="px-4 py-1.5 rounded-lg text-xs font-medium bg-telegram-primary text-white hover:bg-telegram-primary/90 transition">
                Done
              </button>
            </div>
          </motion.div>

  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const res = await invoke<ShareInfo>("cmd_create_share", {
        ...
        messageId: file.id, // In Cicem Drive, file.id is the message id
        fileName: file.name,
        fileSize: file.size,
        password: pwdParam,
        expiryHours,
        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-4 max-h-[75vh]">
          ...
          {!shareInfo ? (
            <>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between py-1">
                  ...
                  <button
                    ...
                    className={`relative w-10 h-5.5 rounded-full transition-colors duration-200 shrink-0 ${requirePassword ? "bg-telegram-primary" : "bg-telegram-border"}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 w-4.5 h-4.5 rounded-full bg-white shadow transition-transform duration-200 ${requirePassword ? "translate-x-4.5" : "translate-x-0"}`} />
                  </button>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["1h", "1d", "7d"] as const).map((type) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                        expiryType === type ? "bg-telegram-primary border-telegram-primary text-white" : "bg-telegram-surface border-telegram-border text-telegram-text hover:bg-telegram-hover"
                      }`}
                    >
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                      expiryType === "never" ? "bg-telegram-primary border-telegram-primary text-white" : "bg-telegram-surface border-telegram-border text-telegram-text hover:bg-telegram-hover"
                    }`}
                  >
                    Never
                  </button>
                  <button
                    ...
                    className={`col-span-2 px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                      expiryType === "custom" ? "bg-telegram-primary border-telegram-primary text-white" : "bg-telegram-surface border-telegram-border text-telegram-text hover:bg-telegram-hover"
                    }`}
                  >
                    Custom Hours
                  </button>
              ...
              >
                {loading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : "Generate Shareable Link"}
              </button>
            </>
          ) : (
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <input type="text" readOnly value={getDisplayLink()} className="flex-1 bg-telegram-surface/50 border border-telegram-border rounded-lg px-3 py-2 text-sm text-telegram-text focus:outline-none select-all" />
                  <button
                    onClick={handleCopy}
                    className={`px-3 py-2 rounded-lg border flex items-center justify-center transition-all ${
                      copied ? "bg-emerald-500 border-emerald-500 text-white" : "bg-telegram-hover border-telegram-border text-telegram-text hover:bg-white/10"
                    }`}
                  >
              ...
              <div className="bg-telegram-hover/30 border border-telegram-border/50 rounded-lg p-3 space-y-2">
                ...
                <p className="text-xs text-telegram-subtext leading-relaxed">To share with someone else on your Tailscale network or local WiFi, enter your machine's IP address or hostname below:</p>
              ...
              <button onClick={onClose} className="w-full bg-telegram-hover hover:bg-white/10 text-telegram-text text-sm font-medium py-2 rounded-lg transition-colors border border-telegram-border">
                Done
              </button>
            </div>
          )}

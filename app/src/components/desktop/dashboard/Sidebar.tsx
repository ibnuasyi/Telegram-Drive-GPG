...
export function Sidebar({ folders, activeFolderId, setActiveFolderId, onDrop, onDelete, onRename, onToggleVisibility, onExportInvite, onCreate, isSyncing, isConnected, onSync, onLogout, bandwidth }: SidebarProps) {
  ...
  return (
    <aside className="w-64 bg-telegram-surface border-r border-telegram-border flex flex-col" onClick={(e) => e.stopPropagation()}>
      <div className="p-4 flex items-center gap-2">
        ...
        <span className="font-bold text-lg text-telegram-text tracking-tight">Cicem Drive</span>
      </div>

      {/* Scrollable folder list */}
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto min-h-0">
        <SidebarItem icon={HardDrive} label="Saved Messages" active={activeFolderId === null} onClick={() => setActiveFolderId(null)} onDrop={(e: React.DragEvent) => onDrop(e, null)} folderId={null} />
        {folders.map((folder) => (
          <SidebarItem
            key={folder.id}
            icon={Folder}
      ...
      <div className="p-4 border-t border-telegram-border">
        ...
        <div className="flex gap-2 mt-4">
          ...
          <button onClick={onLogout} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-red-500 hover:text-red-600 bg-red-500/10 hover:bg-red-500/20 rounded-lg transition-colors" title="Sign Out">

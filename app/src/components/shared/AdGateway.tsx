...
export function AdGateway({ onContinue }: AdGatewayProps) {
  ...
  const handleSmartLinkClick = useCallback(
  ...
  return (
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className={`auth-glass rounded-3xl shadow-2xl w-full max-w-md text-center ${isMobile ? "p-5" : "p-8"}`}>
        {/* Logo */}
        <div className={`mx-auto flex items-center justify-center ${isMobile ? "mb-4" : "mb-6"}`}>
          <img src="/logo.svg" className={`drop-shadow-lg ${isMobile ? "w-16 h-16" : "w-20 h-20"}`} alt="Cicem Drive Logo" />
        </div>

        {/* Title */}
        <h1 className={`font-bold text-white mb-2 tracking-tight ${isMobile ? "text-lg" : "text-2xl"}`}>Welcome to Cicem Drive</h1>
        <p className="text-sm text-white/60 font-medium mb-8">Tap below to continue to your files</p>
        ...
        <button
          ...
          className={`w-full rounded-2xl font-bold flex items-center justify-center gap-2.5 transition-all duration-300 ${isMobile ? "py-3.5 text-sm" : "py-5 text-base"} ${
            hasClicked ? "bg-white/5 text-white/30 border border-white/10 cursor-default" : "bg-gradient-to-r from-telegram-primary to-blue-500 text-black hover:shadow-xl hover:shadow-blue-500/30 active:scale-[0.98]"
          }`}
        >
          {isOpening ? (
            <>
        ...
        <p className="text-[11px] text-white/30 mt-5 leading-relaxed">This helps support development and keeps Cicem Drive free. You'll only see this once.</p>

        {/* Screen-reader countdown announcements */}
        <div aria-live="polite" className="sr-only">
          {skipCountdown > 0 ? `Skip available in ${skipCountdown} ${skipCountdown === 1 ? "second" : "seconds"}` : "Skip now available"}
        </div>

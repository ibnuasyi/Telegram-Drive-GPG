...
export async function nativeShareOrCopy(name: string, sizeStr: string, link: string, onCopy?: (link: string) => void): Promise<void> {
  const canShare = typeof navigator !== "undefined" && typeof navigator.share === "function";
  if (canShare) {
    try {
      await navigator.share({
        title: `Shared file: ${name}`,
        text: `Download "${name}" (${sizeStr}) via Cicem Drive`,
        url: link,
      });
      return;
    } catch (e: any) {
